...
use revolt_models::v0::{
    self, BulkMessageResponse, DataMessageSend, Embed, MessageAuthor, MessageFlags,
    MessageSearchHas, MessageSort, MessageWebhook, PushNotification, ReplyIntent, SendableEmbed,
    Text,
};
...
impl Message {
    ...
    pub async fn add_reaction(&self, db: &Database, user: &User, emoji: &str) -> Result<()> {
        ...
        // Server-level hooks: starboard + reaction roles
        if let Ok(channel) = db.fetch_channel(&self.channel).await {
            if let Channel::TextChannel { server, .. } = &channel {
                if let Ok(server_obj) = db.fetch_server(server).await {
                    // Starboard
                    if let Some(ref starboard) = server_obj.starboard {
                        if emoji == starboard.emoji {
                            let count = self.reactions.get(emoji).map(|s| s.len()).unwrap_or(0) + 1;
                            if count as u32 >= starboard.threshold {
                                let content = format!(
                                    "⭐ **{}** | <#{}>\n\n{}",
                                    count,
                                    self.channel,
                                    self.content.as_deref().unwrap_or("*(attachment)*")
                                );
                                let _ = SystemMessage::Text { content }
                                    .into_message(starboard.channel.clone())
                    ...
                    // Reaction roles: assign role on reaction
                    for mapping in &server_obj.reaction_roles {
                        if mapping.message_id == self.id && mapping.emoji == emoji {
                            if let Ok(mut member) = db.fetch_member(server, &user.id).await {
                                if !member.roles.contains(&mapping.role_id) {
                                    member.roles.push(mapping.role_id.clone());
                                    let partial = crate::PartialMember {
                                        roles: Some(member.roles.clone()),
                                        ..Default::default()
                                    };
                                    let _ = db.update_member(&member.id, &partial, vec![]).await;
                                }
                            }
                            break;
                        }
        ...
        if let Ok(channel) = db.fetch_channel(&self.channel).await {
            if let Channel::TextChannel { server, .. } = &channel {
                if let Ok(server_obj) = db.fetch_server(server).await {
                    for mapping in &server_obj.reaction_roles {
                        if mapping.message_id == self.id && mapping.emoji == emoji {
                            if let Ok(mut member) = db.fetch_member(server, user).await {
                                if member.roles.contains(&mapping.role_id) {
                                    member.roles.retain(|r| r != &mapping.role_id);
                                    let partial = crate::PartialMember {
                                        roles: Some(member.roles.clone()),
                                        ..Default::default()
                                    };
