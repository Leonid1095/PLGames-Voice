...
/// Decrypt the content field of a single message
fn decrypt_message(mut msg: Message) -> Message {
    msg.content = message_encryption::decrypt_option(&msg.content);
    msg
}

/// Decrypt the content field of a vec of messages
fn decrypt_messages(messages: Vec<Message>) -> Vec<Message> {
    messages.into_iter().map(decrypt_message).collect()
}
...
impl AbstractMessages for MongoDb {
    ...
    async fn insert_message(&self, message: &Message) -> Result<()> {
        ...
        msg.content = message_encryption::encrypt_option(&msg.content);
        query!(self, insert_one, COL, &msg).map(|_| ())
    }
    ...
    async fn fetch_messages(&self, query: MessageQuery) -> Result<Vec<Message>> {
        ...
        if let Some(has) = query.filter.has {
            match has {
                ...
                MessageSearchHas::Embed => {
                    filter.insert("embeds.0", doc! { "$exists": true });
                }
                MessageSearchHas::Link => {
                    filter.insert(
                        "content",
                        doc! { "$regex": "https?://", "$options": "i" },
                    );
                }
                MessageSearchHas::Mention => {
                    filter.insert("mentions.0", doc! { "$exists": true });
                }
            }
        }
        ...
        match query.time_period {
            MessageTimePeriod::Relative { nearby } => {
                ...
                Ok(decrypt_messages([a, b].concat()))
            }
    ...
    async fn fetch_latest_message_by_user(
        &self,
        channel_id: &str,
        user_id: &str,
        ...
        self.col::<Message>(COL)
            ...
            .try_next()
            .await
            .map(|opt| opt.map(decrypt_message))
            .map_err(|_| create_database_error!("try_next", COL))
    }
}
