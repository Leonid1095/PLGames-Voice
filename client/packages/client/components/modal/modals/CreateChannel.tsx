...
export function CreateChannelModal(
  ...
  async function onSubmit() {
    try {
      const channelType = group.controls.type.value;
      const isVoice = channelType === "Voice";
      const actualType = isForum() && channelType === "Text" ? "Forum" : channelType;
      const description = isVoice && isTrigger() ? TRIGGER_MARKER : undefined;

      const channel = await props.server.createChannel({
        type: actualType as "Text" | "Voice" | "Forum",
            ...
            <Row align gap="md">
              ...
              <Column gap="none">
                ...
                <Text class="body" size="small">
                  <Trans>Structured discussions with threads and tags</Trans>
                </Text>
              </Column>
            </Row>
          </Show>
