...
impl AbstractServerMembers for ReferenceDb {
    ...
    async fn fetch_top_members_by_xp(&self, server_id: &str, limit: u32) -> Result<Vec<Member>> {
        let members = self.server_members.lock().await;
        let mut matching: Vec<Member> = members
            .values()
            .filter(|m| m.id.server == server_id && m.xp > 0)
            .cloned()
            .collect();
