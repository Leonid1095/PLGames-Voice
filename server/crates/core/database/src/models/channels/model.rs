...
auto_derived!(
    ...
    pub enum Channel {
        ...
        TextChannel {
            ...
            /// Forum Information for when this channel is a forum channel
            #[serde(skip_serializing_if = "Option::is_none")]
            forum: Option<ForumInformation>,

            /// Parent channel ID (for forum threads)
            #[serde(skip_serializing_if = "Option::is_none")]
            parent: Option<String>,

            /// Applied tags (for forum threads)
            #[serde(skip_serializing_if = "Option::is_none")]
...
impl Channel {
    ...
    pub async fn create_server_channel(
        ...
        let channel = match data.channel_type {
            ...
            v0::LegacyServerChannelType::Forum => Channel::TextChannel {
                id: id.clone(),
                server: server.id.to_owned(),
                name: data.name,
                description: data.description,
                ...
                nsfw: data.nsfw.unwrap_or(false),
                voice: None,
                forum: Some(v0::ForumInformation::default().into()),
                parent: None,
                tags: None,
                slowmode: None,
            },
    ...
    pub fn remove_field(&mut self, field: &FieldsChannel) {
        match field {
            ...
            FieldsChannel::Forum => match self {
                Self::TextChannel { forum, .. } => {
                    forum.take();
                }
                _ => {}
            },
