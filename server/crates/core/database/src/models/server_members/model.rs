...
auto_derived_partial!(
    ...
    pub struct Member {
        ...
        /// Timestamp of last XP gain (for cooldown)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub last_xp_at: Option<Timestamp>,
...
impl Member {
    ...
    pub async fn create(
        ...
        // Auto-assign roles for new members
        if !server.auto_roles.is_empty() {
            let mut new_roles = member.roles.clone();
            for role_id in &server.auto_roles {
                if server.roles.contains_key(role_id) && !new_roles.contains(role_id) {
                    new_roles.push(role_id.clone());
                }
            }
            if new_roles.len() != member.roles.len() {
