...
use revolt_models::v0::{self, AuditLogActionType};
...
pub async fn edit(
    ...
    let mut partial = PartialServer {
        ...
        welcome_message,
        leveling_enabled,
        reaction_roles: reaction_roles.map(|v| {
            v.into_iter()
                .map(|r| revolt_database::ReactionRoleMapping {
    ...
    revolt_database::log_audit_action(
        ...
        &server.id,
        &user.id,
        AuditLogActionType::ServerUpdate,
        "server",
        &server.id,
        None,
        None,
