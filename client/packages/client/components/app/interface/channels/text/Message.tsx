...
export function Message(props: Props) {
  ...
  const handleDblClick = (e: MouseEvent) => {
    // Don't react if text is selected
    const selection = window.getSelection();
    if (selection && selection.toString().length > 0) return;
    // Don't react if clicking on code blocks, links, or buttons
    const target = e.target as HTMLElement;
    if (target.closest("pre, a, button, .reactions")) return;
    // Don't react on system messages
    if (props.message.systemMessage) return;
    ...
    <MessageContainer
      ...
      info={
        <Switch fallback={<div />}>
          ...
          >
            <NewUser>
              <Tooltip content={t`New to PLG Voice`} placement="top">
                <Symbol size={16} fill>
                  spa
                </Symbol>
              </Tooltip>
