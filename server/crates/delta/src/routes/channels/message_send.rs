...
pub async fn message_send(
    ...
    // Enforce slowmode rate limit
    if let revolt_database::Channel::TextChannel { slowmode: Some(interval), .. } = &channel {
        if *interval > 0 && !permissions.has_channel_permission(ChannelPermission::ManageMessages) {
    ...
    // Auto-moderation: blocked words check
    if let Some(server) = query.server_ref() {
        if let Some(ref automod) = server.automod {
    ...
    // Award XP if leveling is enabled for this server
    if let Some(server) = query.server_ref() {
        if server.leveling_enabled {
            if let Ok(member) = db.fetch_member(&server.id, &user.id).await {
                let cooldown_passed = member.last_xp_at
                    .as_ref()
                    .map(|t| {
                        let ts_str = t.to_string();
                        chrono::DateTime::parse_from_rfc3339(&ts_str)
                            .map(|dt| (Utc::now() - dt.with_timezone(&Utc)).num_seconds() >= 60)
                            .unwrap_or(true)
                    })
                    .unwrap_or(true);

                if cooldown_passed {
                    let new_xp = member.xp + 15 + (rand::random::<i64>() % 11); // 15-25 XP
                    let partial = revolt_database::PartialMember {
                        xp: Some(new_xp),
                        last_xp_at: Some(iso8601_timestamp::Timestamp::now_utc()),
                        ..Default::default()
                    };
                    db.update_member(&member.id, &partial, vec![]).await.ok();
                }
            }
        }
    }
