...
**План:** Объединить в 1-2 прохода с использованием одного `createMemo`.

**Тесты:** `client/packages/stoat.js/tests/member-filtering.test.ts`
- Тест: `результат фильтрации идентичен до и после оптимизации`
- Тест: `производительность: 1000 участников обрабатываются < 50мс`
...
| Unit-тесты | 0 | 20+ |
| E2E-тесты | 1 | 8+ |
| A11y нарушений (axe-core) | ~10 | 0 критических |
| Initial bundle size | ~2.5MB | < 1.5MB |
| `as any` в коде | ~10 | 0 |
| Error Boundaries | 0 | 3+ (Messages, TextEditor, ServerSidebar) |
