...
To ensure reliability for users on PLG Voice, clients should implement the following rigid specification for maintaining a session. At a high-level, it should be implemented as a state machine.
...
| Listener                                     | Transition          |
| -------------------------------------------- | ------------------- |
| Connected (and initial data loaded)          | `SOCKET_CONNECTED`  |
| Connection dropped                           | `SOCKET_DROPPED`    |
...
- While not strictly relevant to session lifecycle, if you encounter BlockedByShield during login, you should provide a relevant support message.
